...
const CHAT_SPAM_RESET_RETENTION_MS = CHAT_SPAM_COOLDOWN_DURATION_MS + CHAT_SPAM_REPEAT_WINDOW_MS + 60000;
const CHAT_SPAM_RESET_STATE_PREFIX = 'chat_spam_reset:';
const chatSpamCooldownByUser = new Map();
const chatSpamResetAtByUser = new Map();
...
function pruneChatSpamResetMarkers(now = Date.now()) {
  for (const [key, resetAt] of chatSpamResetAtByUser) {
    if (!Number(resetAt) || now - Number(resetAt) > CHAT_SPAM_RESET_RETENTION_MS) chatSpamResetAtByUser.delete(key);
  }
  while (chatSpamResetAtByUser.size > CHAT_SPAM_COOLDOWN_MAX_USERS) {
    chatSpamResetAtByUser.delete(chatSpamResetAtByUser.keys().next().value);
  }
}

function getChatSpamResetAt(name, now = Date.now()) {
  const key = getChatSpamCooldownKey(name);
  if (!key) return 0;
  const resetAt = Number(chatSpamResetAtByUser.get(key) || 0);
  if (!resetAt || now - resetAt > CHAT_SPAM_RESET_RETENTION_MS) {
    chatSpamResetAtByUser.delete(key);
    return 0;
  }
  return resetAt;
}

function setChatSpamResetAt(name, resetAt = Date.now()) {
  const key = getChatSpamCooldownKey(name);
  const stamp = Math.max(0, Number(resetAt) || Date.now());
  if (!key || !stamp) return 0;
  const previous = Number(chatSpamResetAtByUser.get(key) || 0);
  const next = Math.max(previous, stamp);
  chatSpamResetAtByUser.delete(key);
  chatSpamResetAtByUser.set(key, next);
  pruneChatSpamResetMarkers();
  return next;
}

async function persistChatSpamResetMarker(name, resetAt) {
  const key = getChatSpamCooldownKey(name);
  const stamp = Math.max(0, Number(resetAt) || 0);
  if (!key || !stamp) return false;
  await saveAdminState(`${CHAT_SPAM_RESET_STATE_PREFIX}${key}`, { name: normalizeText(name, '').slice(0, 120), resetAt: stamp });
  return true;
}

async function loadChatSpamResetMarkersFromDb() {
  const now = Date.now();
  const cutoff = now - CHAT_SPAM_RESET_RETENTION_MS;
  try {
    const result = await queryDbWithRetry(
      'SELECT state_key, data FROM admin_state WHERE state_key LIKE $1',
      [`${CHAT_SPAM_RESET_STATE_PREFIX}%`],
      { attempts: 2, label: 'CHAT SPAM RESET READ' }
    );
    const staleKeys = [];
    for (const row of result.rows || []) {
      const data = row && row.data && typeof row.data === 'object' ? row.data : {};
      const resetAt = Math.max(0, Number(data.resetAt) || 0);
      const name = normalizeText(data.name, '').slice(0, 120) || String(row.state_key || '').slice(CHAT_SPAM_RESET_STATE_PREFIX.length);
      if (!resetAt || resetAt <= cutoff || !name) {
        if (row && row.state_key) staleKeys.push(row.state_key);
        continue;
      }
      setChatSpamResetAt(name, resetAt);
    }
    if (staleKeys.length) {
      deferServerTask('CHAT SPAM RESET CLEANUP', () => pool.query('DELETE FROM admin_state WHERE state_key = ANY($1::text[])', [staleKeys]), 0);
    }
  } catch (err) {
    console.error('[CHAT SPAM RESET READ ERROR]:', err && err.message ? err.message : err);
  }
}
...
function setChatSpamCooldownState(name, state) {
  ...
  const nextState = { ...state, name: normalizeText(name, '').slice(0, 120) || key };
  chatSpamCooldownByUser.delete(key);
  chatSpamCooldownByUser.set(key, nextState);
...
function buildPublicChatSpamState(name, state, now = Date.now()) {
  const userName = normalizeText(name, '').slice(0, 120);
  const activeUntil = Math.max(0, Number(state && state.activeUntil) || 0);
  if (!userName || activeUntil <= now) return null;
  return {
    name: userName,
    active: true,
    activeRemainingSeconds: Math.max(1, Math.ceil((activeUntil - now) / 1000))
  };
}

function getPublicChatSpamStates(now = Date.now()) {
  pruneChatSpamCooldowns(now);
  const states = [];
  for (const [key, state] of chatSpamCooldownByUser) {
    if (!state || Number(state.activeUntil || 0) <= now) continue;
    const publicState = buildPublicChatSpamState(state.name || key, state, now);
    if (publicState) states.push(publicState);
  }
  return states;
}

function emitChatSpamPublicSnapshot(socket) {
  if (!socket || !socket.connected) return;
  socket.emit('chat_spam_public_state', { replace:true, states:getPublicChatSpamStates() });
}

function emitChatSpamPublicSnapshotToAll() {
  io.emit('chat_spam_public_state', { replace:true, states:getPublicChatSpamStates() });
}

function emitChatSpamPublicDelta(name, state = null, extra = {}) {
  const userName = normalizeText(name, '').slice(0, 120);
  if (!userName) return;
  const publicState = state ? buildPublicChatSpamState(userName, state) : null;
  io.emit('chat_spam_public_state', {
    replace:false,
    state: publicState || { name:userName, active:false },
...
function countRecentRepeatedChatMessages(senderName, comparableText, now = Date.now()) {
  ...
  const resetAt = getChatSpamResetAt(senderName, now);
  let count = 0;
  for (let index = messageHistory.length - 1; index >= 0; index--) {
    ...
    if (resetAt && sentAt && sentAt <= resetAt) break;
...
function deriveChatSpamCooldownFromHistory(senderName, now = Date.now()) {
  ...
  const resetAt = getChatSpamResetAt(senderName, now);
  const relevantSince = Math.max(now - CHAT_SPAM_COOLDOWN_DURATION_MS - CHAT_SPAM_REPEAT_WINDOW_MS, resetAt || 0);
  ...
  for (let index = 0; index < messageHistory.length; index++) {
    ...
    const sentAt = Date.parse(entry.time || '') || 0;
    if (!sentAt || sentAt < relevantSince || (resetAt && sentAt <= resetAt)) continue;
...
function recoverAllChatSpamCooldownStatesFromHistory() {
  const now = Date.now();
  if (!Array.isArray(messageHistory) || !messageHistory.length) return 0;
  const cutoff = now - CHAT_SPAM_COOLDOWN_DURATION_MS - CHAT_SPAM_REPEAT_WINDOW_MS;
  const names = new Set();
  for (let index = messageHistory.length - 1; index >= 0; index--) {
    const entry = messageHistory[index];
    if (!entry || typeof entry !== 'object') continue;
    const sentAt = Date.parse(entry.time || '') || 0;
    if (sentAt && sentAt < cutoff) break;
    const name = normalizeText(entry.user, '').slice(0, 120);
...
async function notifyChatSpamAcrossInstances(name, state, extra = {}) {
  const userName = normalizeText(name, '').slice(0, 120);
  const activeUntil = Math.max(0, Number(state && state.activeUntil) || 0);
  const cleared = extra.cleared === true;
  const eventAt = Math.max(0, Number(extra.eventAt) || Date.now());
  const resetAt = cleared ? Math.max(0, Number(extra.resetAt) || eventAt) : 0;
  if (!userName || (!activeUntil && !cleared)) return;
  const payload = {
...
async function clearChatSpamProtectionForUser(name, options = {}) {
  const userName = normalizeText(name, '').slice(0, 120);
  const key = getChatSpamCooldownKey(userName);
  if (!userName || !key) return { success:false, message:'Missing target user.' };

  const resetAt = setChatSpamResetAt(userName, Math.max(0, Number(options.resetAt) || Date.now()));
  const wasActive = !!getChatSpamCooldownState(userName, resetAt);
  chatSpamCooldownByUser.delete(key);

  if (options.persist !== false) await persistChatSpamResetMarker(userName, resetAt);
  emitChatSpamCooldownStateToUser(userName, null, { cleared:true, resetAt });
  emitChatSpamPublicDelta(userName, null, { cleared:true });
  if (options.notify !== false) {
    deferServerTask('CHAT SPAM CLEAR NOTIFY', () => notifyChatSpamAcrossInstances(userName, null, { cleared:true, resetAt, eventAt:resetAt }), 0);
  }
  return { success:true, targetName:userName, resetAt, wasActive };
}
...
const KEEP_ALIVE_URLS = [
  "https://psn-content-jb05.onrender.com",
];
...
function cleanupAdminPostgresBackupTokens(now = Date.now()) {
  for (const [token, entry] of adminPostgresBackupTokens) {
    if (!entry || Number(entry.expiresAt || 0) <= now) adminPostgresBackupTokens.delete(token);
  }
}
...
app.get('/api/admin/postgres-backup', (req, res) => {
  ...
  if (!grant || Number(grant.expiresAt || 0) <= Date.now()) {
    if (token) adminPostgresBackupTokens.delete(token);
    return res.status(403).json({ ok: false, error: 'Backup link is invalid or expired.' });
  }
  adminPostgresBackupTokens.delete(token); // single use
...
async function initDb() {
  await pool.query(`
    ...
    CREATE TABLE IF NOT EXISTS user_game_play_together (
      user_name TEXT NOT NULL REFERENCES users(name) ON DELETE CASCADE,
      title_id TEXT NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS idx_user_game_play_together_title ON user_game_play_together(title_id, created_at DESC);
  ...
  await loadChatSpamResetMarkersFromDb();
  recoverAllChatSpamCooldownStatesFromHistory();
...
function setCachedGamePlayersSummary(userName, titleId, summary = {}) {
  ...
  gamePlayersSummaryCache.set(key, {
    ...
    summary: {
      ...
      playTogetherCount: Math.max(0, Number(summary.playTogetherCount) || 0),
      playTogetherFriendCount: Math.max(0, Number(summary.playTogetherFriendCount) || 0),
      playTogetherOwnerCount: Math.max(0, Number(summary.playTogetherOwnerCount) || 0),
      playTogetherOwnerFriendCount: Math.max(0, Number(summary.playTogetherOwnerFriendCount) || 0),
      viewerOwns: summary.viewerOwns === true,
      viewerPlayTogether: summary.viewerPlayTogether === true,
...
async function notifyPlayTogetherPlayers(actorName, rawTitleId, rawTitle) {
  ...
  const result = await queryDbWithRetry(
    ...
    [titleId, actor],
    { attempts: 2, label: 'PLAY TOGETHER MATCH' }
  );
...
async function getPlayTogetherDiscoveryForUser(userName, rawAudience) {
  ...
  const result = await queryDbWithRetry(
    ...
    [viewerName, audience],
    { attempts: 2, label: audience === 'friends' ? 'PLAY TOGETHER FRIEND STATE' : 'PLAY TOGETHER OTHER PLAYER STATE' }
  );
...
async function getPlayTogetherCommunityTitleIds(userName) {
  ...
  const result = await queryDbWithRetry(
    ...
    [viewerName],
    { attempts: 2, label: 'PLAY TOGETHER COMMUNITY GAME IDS' }
  );
...
function hydrateGamePlayerEntry(row, titleId) {
  ...
  return {
    ...
    level: Math.max(1, Number(publicUser.level) || 1),
    ...
    playTogether: row && row.play_together === true,
...
async function getGamePlayersSummaryForUser(userName, rawTitleId) {
  const viewerName = normalizeText(userName, '');
  const titleId = normalizeGamePlayersTitleId(rawTitleId);
  if (!viewerName || !titleId) return { count: 0, friendCount: 0, otherCount: 0, playTogetherCount: 0, playTogetherFriendCount: 0, playTogetherOwnerCount: 0, playTogetherOwnerFriendCount: 0, viewerOwns: false, viewerPlayTogether: false, preview: [] };
    ...
    `WITH viewer_friends AS (
     ...
     play_together AS (
       SELECT
         interest.user_name,
         (friend.name IS NOT NULL) AS is_friend,
         (ownership.user_name IS NOT NULL) AS owns_game
       FROM user_game_play_together interest
       LEFT JOIN viewer_friends friend ON friend.name = interest.user_name
       LEFT JOIN user_game_ownership ownership
         ON ownership.user_name = interest.user_name
        AND ownership.title_id = interest.title_id
        AND (ownership.in_library = TRUE OR ownership.downloaded = TRUE)
       WHERE interest.title_id = $2
         AND interest.user_name <> $1
     ),
     metrics AS (
       SELECT
         (SELECT COUNT(*)::int FROM owners) AS total_count,
         (SELECT COUNT(*)::int FROM owners WHERE is_friend) AS friend_count,
         (SELECT COUNT(*)::int FROM play_together) AS play_together_count,
         (SELECT COUNT(*)::int FROM play_together WHERE is_friend) AS play_together_friend_count,
         (SELECT COUNT(*)::int FROM play_together WHERE owns_game) AS play_together_owner_count,
         (SELECT COUNT(*)::int FROM play_together WHERE owns_game AND is_friend) AS play_together_owner_friend_count,
         EXISTS (
         ...
         EXISTS (
           SELECT 1 FROM user_game_play_together mine
           WHERE mine.user_name = $1 AND mine.title_id = $2
         ) AS viewer_play_together
     )
     SELECT
       owner.user_name,
       owner.in_library,
       ...
       owner.is_friend,
       CASE
         WHEN owner.user_name IS NULL THEN FALSE
         ELSE EXISTS (SELECT 1 FROM play_together pt WHERE pt.user_name = owner.user_name)
       END AS play_together,
       metrics.total_count,
       metrics.friend_count,
       metrics.play_together_count,
       metrics.play_together_friend_count,
       metrics.play_together_owner_count,
       metrics.play_together_owner_friend_count,
       metrics.viewer_owns,
       metrics.viewer_play_together
     FROM metrics
     LEFT JOIN LATERAL (
       SELECT candidate.*
       FROM owners candidate
       ORDER BY
         CASE WHEN EXISTS (SELECT 1 FROM play_together pt WHERE pt.user_name = candidate.user_name) THEN 0 ELSE 1 END,
         candidate.is_friend DESC,
         candidate.user_name ASC
       LIMIT 3
     ) owner ON TRUE`,
    [viewerName, titleId],
  ...
  const first = rows[0] || {};
  const count = Math.max(0, Number(first.total_count) || 0);
  const friendCount = Math.max(0, Number(first.friend_count) || 0);
  const playTogetherCount = Math.max(0, Number(first.play_together_count) || 0);
  const playTogetherFriendCount = Math.max(0, Number(first.play_together_friend_count) || 0);
  const playTogetherOwnerCount = Math.max(0, Number(first.play_together_owner_count) || 0);
  const playTogetherOwnerFriendCount = Math.max(0, Number(first.play_together_owner_friend_count) || 0);
  const viewerOwns = first.viewer_owns === true;
  const viewerPlayTogether = first.viewer_play_together === true;
  const summary = {
    count,
    friendCount,
    otherCount: Math.max(0, count - friendCount),
    playTogetherCount,
    playTogetherFriendCount,
    playTogetherOwnerCount,
    playTogetherOwnerFriendCount,
    viewerOwns,
    viewerPlayTogether,
    ...
    result = await queryDbWithRetry(
      `WITH viewer_friends AS (
       ...
       WHERE interest.title_id = $2
         AND interest.user_name <> $1
       ORDER BY
         CASE WHEN friend.name IS NOT NULL THEN 0 ELSE 1 END,
         interest.user_name ASC
       LIMIT $3 OFFSET $4`,
      [viewerName, titleId, limit, offset],
      { attempts: 2, label: 'GAME PLAY TOGETHER PAGE READ' }
    );
  } else {
    result = await queryDbWithRetry(
      `WITH viewer_friends AS (
       ...
       SELECT
         ...
         ownership.downloaded,
         (friend.name IS NOT NULL) AS is_friend,
         FALSE AS play_together,
         COUNT(*) OVER()::int AS group_count
       FROM user_game_ownership ownership
       LEFT JOIN viewer_friends friend ON friend.name = ownership.user_name
       LEFT JOIN user_game_play_together interest
         ON interest.user_name = ownership.user_name AND interest.title_id = ownership.title_id
       WHERE ownership.title_id = $2
         AND ownership.user_name <> $1
         AND (ownership.in_library = TRUE OR ownership.downloaded = TRUE)
         AND interest.user_name IS NULL
         AND (
...
async function resolveChatDeleteTargetsForSocket(socket, rawTargets = []) {
  const targets = [];
  const seenIds = new Set();
  for (const raw of Array.isArray(rawTargets) ? rawTargets : []) {
    const source = raw && typeof raw === 'object' ? raw : { id: raw };
    const id = normalizeText(source.id || source.msgId, '').slice(0, 100);
    if (!id || seenIds.has(id)) continue;
    seenIds.add(id);
    let time = normalizeText(source.time || source.msgTime, '').slice(0, 100);
    if (!time && /^\d{10,17}$/.test(id)) {
      const numericId = Number(id);
      if (Number.isFinite(numericId) && numericId > 0) {
        try { time = new Date(numericId).toISOString(); } catch (err) {}
      }
    }
    targets.push({ id, time });
  }
  if (!targets.length) return { approved: [], deniedIds: [], alreadyDeletedIds: [] };

  const requestedSet = new Set(targets.map(target => target.id));
  const messageById = new Map();
  for (const msg of messageHistory) {
    if (!msg || !msg.time) continue;
    const id = String(new Date(msg.time).getTime());
    if (requestedSet.has(id) && !messageById.has(id)) messageById.set(id, msg);
  }

  const missingTargets = targets.filter(target => !messageById.has(target.id) && target.time);
  if (missingTargets.length) {
    const wantedIds = new Set(missingTargets.map(target => target.id));
  ...
  for (const target of targets) {
    const msg = messageById.get(target.id);
    if (!msg) {
      alreadyDeletedIds.push(target.id);
      continue;
    }
    const isOwner = msg.user === socket.userName;
    if (isOwner || (canModerate && canModerateTarget(socket, msg.user))) {
      approved.push({ id: target.id, time: String(msg.time), user: String(msg.user || ''), isOwner });
    } else {
      deniedIds.push(target.id);
    }
  }
...
async function initProfileSyncNotifications() {
    ...
    try {
      ...
      if (message.channel === 'chat_sync_notify') {
        ...
        if (spam && spam.name) {
          ...
          const now = Date.now();
          const incomingEventAt = Math.max(0, Number(spam.eventAt) || now);
          const incomingResetAt = Math.max(0, Number(spam.resetAt) || incomingEventAt);

          if (spamName && spam.cleared === true) {
            const currentResetAt = getChatSpamResetAt(spamName, now);
            if (!currentResetAt || incomingResetAt >= currentResetAt) {
              chatSpamCooldownByUser.delete(getChatSpamCooldownKey(spamName));
              setChatSpamResetAt(spamName, incomingResetAt);
              emitChatSpamCooldownStateToUser(spamName, null, { cleared:true, resetAt:incomingResetAt, remote:true });
              emitChatSpamPublicDelta(spamName, null, { cleared:true, remote:true });
            }
            if (data.reason === 'spam_state') return;
          }

          const incomingActiveUntil = Math.max(0, Number(spam.activeUntil) || 0);
          const localResetAt = getChatSpamResetAt(spamName, now);
          if (spamName && incomingActiveUntil > now && (!localResetAt || incomingEventAt > localResetAt)) {
            ...
            emitChatSpamPublicDelta(spamName, mergedSpam, { remote:true });
          }
          if (data.reason === 'spam_state') return;
        }
  ...
  try {
    ...
    if (shouldRecoverModerationSync) {
      ...
      deferServerTask('CHAT SPAM LISTEN RECOVERY', () => { recoverAllChatSpamCooldownStatesFromHistory(); const recovered = recoverLocalChatSpamCooldownStates(); emitChatSpamPublicSnapshotToAll(); return recovered; }, 0);
    }
...
async function getUserNotificationHistory(userName, afterId = 0) {
  ...
  return { items: result.rows.map(serializeUserNotificationRow).filter(Boolean), lastReadId, unreadCount, delta: safeAfterId > 0 };
}
...
io.on('connection', (socket) => {
    ...
    try {
      ...
      if (dbUser) {
        ...
        if (match) {
          ...
          emitChatSpamPublicSnapshot(socket);
  ...
  socket.on('request_game_owners', async (data = {}, ack) => {
    ...
    try {
      ...
      if (mode === 'community') {
        const titleIds = await getPlayTogetherCommunityTitleIds(name);
        const payload = { ok: true, mode, titleIds };
        trackBandwidthPayload('game_players_play_together_community', payload, 1);
        respond(payload);
        return;
      }

      if (mode === 'friends' || mode === 'others') {
        const items = await getPlayTogetherDiscoveryForUser(name, mode);
        const payload = { ok: true, mode, items };
        trackBandwidthPayload(mode === 'friends' ? 'game_players_play_together_friends' : 'game_players_play_together_others', payload, 1);
        respond(payload);
        return;
      }
  ...
  socket.on('chat_message', async (msg, callback) => {
    ...
    if (lowerText === '/unspam' || lowerText.startsWith('/unspam ')) {
      if (!canModerate) {
        const blocked = { success: false, reason: 'permission', message: 'Only admins/moderators can use /unspam.' };
        socket.emit('chat_blocked', blocked);
        respond(blocked);
        return;
      }

      const { targetName } = resolveCommandTarget(text.slice('/unspam'.length));
      if (!targetName || !userDatabase[targetName]) {
        const error = { success: false, command: 'unspam', message: 'User not found.' };
        respond(error);
        return;
      }
      if (!canModerateTarget(socket, targetName)) {
        const error = { success: false, command: 'unspam', message: 'You cannot remove spam protection from this user.' };
        respond(error);
        return;
      }

      const result = await clearChatSpamProtectionForUser(targetName);
      if (result.success) {
        await addModerationLog('chat_spam_clear', `Removed spam protection from ${targetName} via chat command`, { targetName, resetAt: result.resetAt, wasActive: result.wasActive }, senderName || 'Moderator');
      }
      respond({ command: 'unspam', ...result });
      return;
    }
    ...
    try {
      ...
      if (!canModerate && spamComparableText) {
        if (spamRepeatCount === CHAT_SPAM_WARNING_COUNT) {
          ...
          emitChatSpamPublicDelta(senderName, spamState, { activated:true });
  ...
  // --------------------------------------------------------------------------
  socket.on('admin_create_postgres_backup', async (data, callback) => {
    const respond = typeof callback === 'function' ? callback : () => {};
    try {
      if (socket.isAdmin !== true) return respond({ success: false, message: 'Admin only.' });
      if (adminPostgresBackupActive) return respond({ success: false, message: 'Another PostgreSQL backup is already running.' });
      cleanupAdminPostgresBackupTokens();
      const token = crypto.randomBytes(32).toString('hex');
      adminPostgresBackupTokens.set(token, {
        actor: socket.userName || 'Admin',
        expiresAt: Date.now() + ADMIN_POSTGRES_BACKUP_TOKEN_TTL_MS
      });
      respond({
        success: true,
        url: `/api/admin/postgres-backup?token=${encodeURIComponent(token)}`,
      ...
      respond({ success: false, message: 'Could not prepare PostgreSQL backup.' });
    }
  });
  ...
  socket.on('delete_messages', async (data = {}, callback) => {
    ...
    const rawTargets = Array.isArray(data && data.messages) && data.messages.length
      ? data.messages
      : (Array.isArray(data && data.msgIds) ? data.msgIds.map(id => ({ id })) : []);
    ...
    try {
      ...
      const racedAlreadyDeletedIds = approved.map(entry => entry.id).filter(id => !deletedIdSet.has(id));
      ...
      respond({
        ...
        deletedIds,
        alreadyDeleted: staleIds.length,
        alreadyDeletedIds: staleIds,
  ...
  socket.on('delete_message', async (data = {}, callback) => {
    ...
    try {
      ...
      if (isPinned) {
        pinnedMessages = pinnedMessages.filter(p => !p || String(p.id) !== msgId);
